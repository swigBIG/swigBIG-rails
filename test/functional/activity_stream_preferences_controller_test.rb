...
class ActivityStreamPreferencesControllerTest < ActionController::TestCase
  ...
  def test_all_checked_should_save_no_data
    ...
    post :create, :bar_id => bars(:aaron).id, :locations => locations
    assert_equal(0, ActivityStreamPreference.find(:all, :conditions => { :bar_id => bars(:aaron)}).size)
  end

  def test_none_checked_should_save_all_data
    login_as :aaron
    post :create, :bar_id => bars(:aaron).id 
    assert_equal(18, ActivityStreamPreference.find(:all, :conditions => 
      { :bar_id => bars(:aaron)}).size)
  end
  ...
  def test_view_should_contain_atom_url
    ...
    a = "http://test.host/feeds/your_activities/#{bars(:aaron).activity_stream_token}"
