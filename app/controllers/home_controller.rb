  ...
  def main
    ...
    if params[:radius].blank? and params[:search].blank?
      ...
      if !params[:search][:zip_code_contains].blank? and !params[:radius].blank?
        ...
        @geo = @geos.first
        @search = Swig.near("#{@geo.latitude},#{@geo.longitude}", params[:radius], :order => :distance).where(swig_day: Date.today.to_time.in_time_zone.strftime("%A").to_s).search(params[:search])
        @swigs = @search
        ...
        @geo = City.find(params[:id])
        @swigs = @search.where(city: @city.name.to_s, status: "active", swig_day: Time.zone.now.strftime("%A").to_s)
      end
    elsif !params[:radius].blank?
#      @city = City.find(params[:id])
  ...
  def city
    ...
    if params[:radius].blank? and params[:search].blank?
      ...
      @search = Swig.search(params[:search])
      @swigs = @search.where(city: @city.name, status: "active", swig_day: Date.today.to_time.in_time_zone.strftime("%A").to_s)
    elsif !params[:search].blank? or !params[:radius].blank?
      @city_bar = Bar.where(city: @city.name, id: swig_bars)
#      @city = City.find(params[:id])
      @loyalty = Loyalty.all
      ...
      if !params[:search][:zip_code_contains].blank? and !params[:radius].blank?
        ...
        @geo = @geos.first
        @search = Swig.near("#{@geo.data["geometry"]["location"]["lat"]},#{@geo.data["geometry"]["location"]["lng"]}", params[:radius], :order => :distance).where(swig_day: Date.today.to_time.in_time_zone.strftime("%A").to_s).search(params[:search])
        @swigs = @search
      elsif !params[:search][:zip_code_contains].blank?
        @geos = Geocoder.search("#{params[:search][:zip_code_contains]},#{@city}")
        @geo = @geos.first
