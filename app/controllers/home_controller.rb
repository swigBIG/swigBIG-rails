class HomeController < ApplicationController
  ...
  def main
    ...
    if params[:radius].blank? and params[:search].blank?
      ...
      if !params[:search][:zip_code_contains].blank? and !params[:radius].blank?
        @geos = Geocoder.search("#{params[:search][:zip_code_contains]},#{@city}").first
        @geo = @geos.first
        ...
        @city_lat_lng = [@city.name, @geo.latitude, @geo.longitude]
        @search = Swig.search(params[:search])
        @swigs = @search.where(city: @city.name.to_s, zip_code: params[:search][:zip_code_contains], status: "active", swig_day: Time.zone.now.strftime("%A").to_s)
  ...
  def city
    ...
    if params[:radius].blank? and params[:search].blank?
      ...
      @search = Swig.search(params[:search])
      @swigs = @search.where(city: @city.name, status: "active", swig_day: Time.zone.now.strftime("%A").to_s)
    elsif !params[:search].blank? or !params[:radius].blank?
      @city_bar = Bar.where(city: @city.name, id: swig_bars)
      #      @city = City.find(params[:id])
      @loyalty = Loyalty.all
